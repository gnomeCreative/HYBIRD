...
struct Node2 {
    ...
    __host__ __device__ bool isActive(const unsigned int i) const {
        // Treat interface filled/empty as active, so it takes the correct branch during eraseNode()
        return type[i] == LIQUID || type[i] == INTERFACE || type[i] == INTERFACE_FILLED || type[i] == INTERFACE_EMPTY || type[i] == GAS_TO_INTERFACE || type[i] == FLUID_TO_INTERFACE;
    }
