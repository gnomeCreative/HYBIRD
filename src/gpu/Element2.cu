...
void Element2::initialize(const unsigned int index, const double partDensity, const DEMParams &dem_p) {
    ...
    if (wSolver[index]) {
        ...
        wpLocal[index] = wLocal[index] = 2.0*quat2vec( q0adj.multiply(q1) );
    }
    ...
    I = size[index] * 2.0/5.0 * singleMass * radius[index] * radius[index] * tVect(1.0,1.0,1.0);
    // transport components
    for (int n=0; n<size[index]; ++n) {
        I+=singleMass*radius[index] *radius[index] * dem_p.prototypes[size[index]][n].transport();
    }
...
void Element2::generateParticles(const unsigned int e_i, unsigned int& p_i, Particle2 &particles, const DEMParams &dem_p) {
    for (unsigned int i = 0; i < size[e_i]; ++i) {
        ...
        particles.updateCorrected(p_i, *this, e_i, dem_p); // was predicted
        ++p_i;
    }
}
