...
__global__ void d_resetForces(Particle2* d_particles, Element2* d_elements, Wall2* d_walls, Object2 *d_objects) {
    ...
    if (i < d_elements->activeCount) {
        common_resetElementForces(d_elements->activeI[i]);
    }
    if (i < d_walls->count) {
            ...
            for (int dz = -1; dz <= 1; ++dz) {
                ...
                for (unsigned int i = d_particles->PBM[hash]; i < d_particles->PBM[hash+1]; ++i){
                    ...
                    if (d_particles->clusterIndex[n_i] != my_cluster) {
                        ...
                        const tVect an_x0 = d_particles->x0[n_i] - d_particles->x0[a_i];
                        const tVect n_x0 = d_particles->x0[n_i].x;
                        n_x0 = 
                        {
                            abs(an_x0.x) > DEM_P.half_demSize ? n_x0.x - (an_x0.x / abs(an_x0.x) * DEM_P.half_demSize) : n_x0.x,
                            abs(an_x0.y) > DEM_P.half_demSize ? n_x0.y - (an_x0.y / abs(an_x0.y) * DEM_P.half_demSize) : n_x0.y,
                            abs(an_x0.z) > DEM_P.half_demSize ? n_x0.z - (an_x0.z / abs(an_x0.z) * DEM_P.half_demSize) : n_x0.z,
                        };
...
void DEM2::evolveBoundaries() {
    // @todo Do we need to sync back any data from device? e.g. h_walls.FParticle?
    switch (problemName) {
    ...
    syncParamsToDevice(); // DEM.pressureX-Z
#endif
}
...
void DEM2::syncElementsFromDevice() {
    ...
    CUDA_CALL(cudaMemcpy(h_elements.q0, hd_elements.q0, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.q1, hd_elements.q1, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.q2, hd_elements.q2, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.q3, hd_elements.q3, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.q4, hd_elements.q4, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.q5, hd_elements.q5, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.qp0, hd_elements.qp0, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.qp1, hd_elements.qp1, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.qp2, hd_elements.qp2, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.qp3, hd_elements.qp3, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.qp4, hd_elements.qp4, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
    CUDA_CALL(cudaMemcpy(h_elements.qp5, hd_elements.qp5, hd_elements.count * sizeof(tVect), cudaMemcpyDeviceToHost));
